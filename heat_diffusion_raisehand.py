...
def encode_temp_frame():
    ...
    Total typical: 9 + 16224 + ~500*20 ≈ 26 KB
    """
    ...
    header   = struct.pack('<IfB', n_s, brightness_scale, touch_byte)
            ...
            try:
                ...
                if len(raw) >= INPUT_FRAME_SIZE:
                    ...
                    with data_lock:
                        ...
                        if had_touch_prev and not touch_detected and not freeze_input:
                            # 抬手瞬间：冻结，并恢复峰值输入
                            freeze_input = True
                            ...
                            peak_touch_area = 0
                        elif not freeze_input:
                            # 正常更新输入
                            try:
                                input_data.from_numpy(nd); new_data_available = True
                            except: pass
                            # 追踪峰值接触面积
                            if touch_detected and contact_area >= peak_touch_area * 0.9:
