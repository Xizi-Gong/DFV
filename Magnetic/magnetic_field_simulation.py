...
# 每场景对强度归一化参考值的额外缩放（索引对应 SCENE_NAMES）：
#   >1 -> 参考值变大 -> 该场景整体更深；<1 -> 更浅。
#   (cmap 已不再 reversed：norm 小 = 近黑，norm 大 = 亮。)
#   只影响"顶"，不影响地板。
SCENE_REF_SCALE = [2.5, 2.5, 5.5, 1.0, 2.0, 1.2, 1.2]

# 每场景的"黑点"分位数：把该场景自己的弱场端锚到近黑。
#   None = 不减地板。Uniform Field 是常数场，减了会整屏纯黑，必须 None。
#   ↑ 调大 -> 最深处更深（只影响暗端，不动已经调好的亮端）。
SCENE_FLOOR_PCT = [1.0, 1.0, None, 1.0, 5.0, 1.0, 1.0]
...
# 线间距淡出：屏幕上相邻磁感线的像素间距 lpp
line_fade_lo = ti.field(dtype=ti.f32, shape=())   # 间距 ≤ 此值：线完全消失
line_fade_hi = ti.field(dtype=ti.f32, shape=())   # 间距 ≥ 此值：线满强度

line_fade_lo[()] = 1.6
line_fade_hi[()] = 4.0

# 全局线宽缩放：统一乘在每个场景各自调好的 line_thickness 上，
# 只动一个数就能整体变细/变粗，不用逐场景改。
LINE_THICKNESS_SCALE = 0.85
...
def compute_and_render(A: ti.template(), colorf: ti.template(), mu_f: ti.template(),
    ...
    for di, dj in colorf:
        ...
        # 间距不足一两像素时淡出（旧版是实心填充，会在中心糊成一坨）
        fade    = ti.math.smoothstep(line_fade_lo[None], line_fade_hi[None], lpp)
        bg_vis  = is_line * fade
        ...
        # 不再在核内烘焙颜色，改为打包覆盖率，颜色交给 numpy 侧按底色明暗自适应：
        #   ch0 = 普通磁感线覆盖率   ch1 = 高亮线覆盖率   ch2 = 磁体本体标记(0/1)
        colorf[di, dj] = ti.Vector([bg_vis, red_vis, 1.0 if is_magnet_body else 0.0])
...
def set_up_scene(n):
    ...
    line_thickness_field[()] = line_thickness_field[()] * LINE_THICKNESS_SCALE
...
def main(web_ui_only=False, max_frames=None, stats_json=None, initial_direction=False):
    ...
    # 深底 Mako：只在低亮度区间活动，最亮端 L≈40%，亮度预算留给磁感线。
    #   弱场 -> 近黑蓝，强场 -> 深青蓝。想让强场更亮/更暗，挪最后一个色标。
    mako_night = LinearSegmentedColormap.from_list('mako_night', [
        (0.00, '#07080f'),
        (0.13, '#0d1220'),
        (0.21, '#172042'),
        (0.33, '#24316c'),
    ...
    cmap = mako_night          # 注意：不再 reversed

    # 磁感线合成用色（0~1 线性 RGB）
    LUMA           = np.array([0.299, 0.587, 0.114], dtype=np.float32)
    LINE_DARK      = np.array([0.10, 0.11, 0.13], dtype=np.float32)  # 暂留，深底不再用
    LINE_LIGHT     = np.array([0.97, 0.98, 1.00], dtype=np.float32)
    LINE_BODY      = np.array([0.50, 0.50, 0.50], dtype=np.float32)  # 磁体上的线(中灰)
    LINE_BODY_FILL = np.array([0.16, 0.17, 0.20], dtype=np.float32)
    # 双色对：OKLCH 等 L 等 C，只转 H —— 两条线亮度/粗细/辉光完全同权，
    # 高亮线只靠色相区分，不带任何"这里场强更大"的暗示。
    #   halo: L=0.70 C=0.118   core: L=0.93 C=0.050
    #   青(195°) 与品紫(330°) 在这两个 L 上都在 sRGB 色域内，能真正等参数；
    #   青+橙做不到——青的 C 上限被 R=0 那面墙卡死，橙远高于此，等 C 会逼青越界。
    LINE_HALO        = np.array([0.05, 0.71, 0.71], dtype=np.float32)  # #0db5b5  H=195
    LINE_ACCENT_HALO = np.array([0.78, 0.51, 0.76], dtype=np.float32)  # #c883c1  H=330
    LINE_CORE        = np.array([0.76, 0.95, 0.95], dtype=np.float32)  # #c2f3f2  H=195
    LINE_ACCENT      = np.array([0.99, 0.86, 0.97], dtype=np.float32)  # #fcdcf8  H=330
    # 芯线带同色相的极淡 C(0.05)，不是白：白芯会在 screen 合成里把 halo 的色相冲掉。

    CORE_ALPHA     = CORE_ALPHA_ACC = 0.70    # 两条线完全同权
    HALO_GAIN      = HALO_GAIN_ACC  = 0.50
    BLOOM_N = BLOOM_A = (1.4, 6.0, 0.75, 0.30)   # (tight, wide, w_tight, w_wide)

    def bloom(c, tight, wide, wt, ww):
        return np.clip(wt * cv2.GaussianBlur(c, (0, 0), tight)
                     + ww * cv2.GaussianBlur(c, (0, 0), wide), 0.0, 1.0)[:, :, None]

    # 可选：把两色对齐到相同的 Rec.709 数值亮度（OKLab 等 L ≠ 709 等 luma，
    # 青 0.48 / 品紫 0.57）。默认不开——709 对齐会把品紫压得看起来比青暗。
    def match_luma(c, ref):
        return np.clip(c * (float(ref @ LUMA) / float(c @ LUMA)), 0.0, 1.0)
    # LINE_ACCENT_HALO = match_luma(LINE_ACCENT_HALO, LINE_HALO)

    # 手部输入覆盖层（磨砂玻璃质感）——去饱和+柔和外阴影，中性低对比，不填实
    HAND_SHADOW       = np.array([0.09, 0.11, 0.13], dtype=np.float32)  # 柔和外阴影(冷中性暗)
    HAND_FROST        = np.array([0.93, 0.95, 0.95], dtype=np.float32)  # 中性磨砂(不再冷青白)
    HAND_SHADOW_A     = 0.10   # 外阴影强度
    HAND_SHADOW_SIGMA = 9.0    # 外阴影扩散(px)
    HAND_DESAT        = 0.20   # 内部去饱和(磨砂扩散感)
    HAND_FROST_A      = 0.13   # 内部提亮强度(越大越"实"、越挡线)
    HAND_EDGE_SIGMA   = 1.0    # 磨砂 alpha 的软化半径(px)：↑更糊 ↓更锐(去掉勾边后由它交代边界)
    HAND_EDGE_GAMMA   = 2.6    # alpha 边缘陡峭度：↑边界更干脆，1.0=不处理(仍保留抗锯齿)

    # 两点归一化：低分位锚到黑(lo)，高分位锚到色带上的固定点(ref)。
    #   只有一个上锚点时，每个场景的地板由物理决定——单线/螺线管画面内根本
    #   没有弱场，最深处就永远浮着。补一个下锚点让每个场景都用满色带。
    #   ref 用 (P98 - lo) 而不是 P98：这样 P98 像素的最终色调恒为
    #   (1/(1+scale))^0.6，与 lo 无关——加地板不会动你已经调好的亮端。
    INTENSITY_REF_PCT = 98.0
    def recompute_intensity_ref():
        compute_B_field(A_field, B_field)
        compute_magnetic_intensity(B_field, magnetic_intensity_field)
        inten = magnetic_intensity_field.to_numpy()
        # 导体/磁体本体像素只在算"地板"时排除：A 在导体中心是对称峰，中心差分
        # 求 B 会精确抵消成 0，这个假零点会把低分位整个带偏（单线 P0 比 P1 低 370 倍）。
        # ref 必须用全帧——磁体本体正是高场区，剔掉它会把 P98 拉低一大截。
        body = initial_mask.to_numpy()[offset_x:offset_x+disp_res_x,
                                      offset_y:offset_y+disp_res_y] > 0.5
        free = inten[~body] if (~body).any() else inten.ravel()

        pct = SCENE_FLOOR_PCT[initial_scene_number]
        lo  = 0.0 if pct is None else float(np.percentile(free, pct))
        ref = float(np.percentile(inten, INTENSITY_REF_PCT)) - lo + 1e-6
        return lo, ref * SCENE_REF_SCALE[initial_scene_number]
    intensity_lo, intensity_ref = recompute_intensity_ref()
    ...
    while web_ui_only or gui.running:
        ...
        if should_render_frame:
            if vis_magnetic_intensity:
                compute_magnetic_intensity(B_field, magnetic_intensity_field)
                inten = magnetic_intensity_field.to_numpy()
                # 两点归一化：先减地板锚到黑，再按 ref 做软压缩。
                d    = np.maximum(inten - intensity_lo, 0.0)
                norm = (d / (d + intensity_ref)) ** 0.6

                base = cmap(norm)[:, :, :3].astype(np.float32)
            else:
                # 强度层关闭时给一块深底
                base = np.full((disp_res_x, disp_res_y, 3), 0.04, dtype=np.float32)

            if vis_magnetic_field_line:
                compute_and_render(A_field, color_field, mu_field,
                                   highlight_A_field_gpu, highlight_count_gpu[()])
                packed    = color_field.to_numpy().astype(np.float32)
                line_cov  = packed[:, :, 0:1]     # 普通线覆盖率
                hi_cov    = packed[:, :, 1:2]     # 高亮线覆盖率
                body_flag = packed[:, :, 2:3]     # 磁体本体

                # 磁体本体填充：深底下这层几乎看不见，留待后续单独调
                body_a = 0.14 * body_flag
                base = base * (1.0 - body_a) + LINE_BODY_FILL * body_a

                # 磁体本体上的线压成中灰弱化，其余用亮芯
                line_rgb = LINE_CORE * (1.0 - 0.6 * body_flag) + LINE_BODY * (0.6 * body_flag)

                # 高亮线所在处把普通线让开：kernel 里 red_vis 是从 bg_vis 派生的，
                # 两层都画会重叠；而 screen 只能提亮不能改色，冷白芯线会把暖橙吃掉。
                line_cov_n = line_cov * (1.0 - hi_cov)

                # screen 合成：两层辉光先铺底，再叠两层芯（芯之间靠覆盖率互斥）
                halo_n = bloom(line_cov_n[:, :, 0], *BLOOM_N)
                halo_a = bloom(hi_cov[:, :, 0],     *BLOOM_A)
                out = 1.0 - (1.0 - base) * (1.0 - LINE_HALO        * halo_n * HALO_GAIN)
                out = 1.0 - (1.0 - out)  * (1.0 - LINE_ACCENT_HALO * halo_a * HALO_GAIN_ACC)
                out = 1.0 - (1.0 - out)  * (1.0 - line_rgb    * line_cov_n * CORE_ALPHA)
                out = 1.0 - (1.0 - out)  * (1.0 - LINE_ACCENT * hi_cov     * CORE_ALPHA_ACC)
                crop_img = np.clip(out, 0.0, 1.0)
            else:
                crop_img = np.clip(base, 0.0, 1.0)

            # ── 手部输入：磨砂玻璃质感 ──
            # 底下的场轻微去饱和+扩散(磨砂)，一圈柔和外阴影做"抬起感"，
            # 整体中性低对比，比发光边更沉稳；深色场线仍从下方透出。
            hand = input_mask.to_numpy()[offset_x:offset_x+disp_res_x,
                                         offset_y:offset_y+disp_res_y].astype(np.float32)
            if hand.max() > 0.5:
                soft = cv2.GaussianBlur(hand, (0, 0), HAND_EDGE_SIGMA)         # 内部软 alpha
                # 把 alpha 的过渡带压陡：中间灰的一段被拉开成接近 0/1，
                # 只在轮廓上留一两像素抗锯齿——边界干脆，但不是一条描边。
                soft = np.clip((soft - 0.5) * HAND_EDGE_GAMMA + 0.5, 0.0, 1.0)
                wide = cv2.GaussianBlur(hand, (0, 0), HAND_SHADOW_SIGMA)
                outer = np.clip(wide - soft, 0.0, 1.0)                         # 轮廓外的软晕

                # 1) 柔和外阴影（抬起感，替代发光）
                sa = (outer * HAND_SHADOW_A)[:, :, None]
                crop_img = crop_img * (1.0 - sa) + HAND_SHADOW * sa
                # 2) 磨砂：先去饱和(往自身灰度靠)，再极淡提亮——像磨砂玻璃扩散底下的场
                lum = (crop_img @ LUMA)[:, :, None]
                da = (soft * HAND_DESAT)[:, :, None]
                crop_img = crop_img * (1.0 - da) + lum * da
                fa = (soft * HAND_FROST_A)[:, :, None]
                crop_img = crop_img * (1.0 - fa) + HAND_FROST * fa
