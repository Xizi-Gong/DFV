...
current_material_name = "Steel"
current_ambient_name  = "Comfortable Room"
...
def handle_ws_message(msg_str):
    ...
    global show_gradient_lines, show_isotherms, paused
...
def encode_temp_frame():
    ...
    Total typical: 8 + 16224 + ~500*20 ≈ 26 KB
    """
    ...
    n_s = len(segs)
    header   = struct.pack('<If', n_s, brightness_scale)
        ...
        while True:
            try:
                ...
                if len(raw) >= INPUT_FRAME_SIZE:
                    nd = np.frombuffer(raw[:INPUT_FRAME_SIZE], dtype=np.uint8).copy().reshape(INPUT_HEIGHT,INPUT_WIDTH).astype(np.float32)/255.0
                    with data_lock:
                        try: input_data.from_numpy(nd); new_data_available = True
                        except: pass
