"""Baseline 2 — SHAPE-PALETTE mode backend.

Reuses the validated solver from fluid_field_simulation.py (imported, NOT modified).
Obstacles are primitive shapes (square/circle/triangle/trapezoid) the user drags into the
field from a sidebar, optionally pinch-resized (no raw heatmap, no karman cylinder).
WebSocket on ws://localhost:8771. Run:  python fluid_field_simulation_shapes.py
"""
import io
import json
import math
...
async def _serve():
    async with websockets.serve(_handler, "localhost", PORT):
        print(f"[WS] shapes baseline ws://localhost:{PORT}")
...
def _poly(cx, cy, size, ang, unit_pts):
...
# 各形状的单位顶点（中心在原点，半尺寸=1，画布系 x右 y下）
_UNIT = {
...
def rasterize_shapes(shapes):
    """shapes: [{type,cx,cy,size,angle}]，归一化坐标(画布系)，size 相对 res_y 的半尺寸。"""
    img = Image.new("L", (res_x, res_y), 0)
    d = ImageDraw.Draw(img)
    for s in shapes:
        cx = float(s.get("cx", 0.5)) * res_x
        cy = float(s.get("cy", 0.5)) * res_y
        size = max(2.0, float(s.get("size", 0.08)) * res_y)
        ang = math.radians(float(s.get("angle", 0.0)))
        t = s.get("type", "circle")
        if t == "circle":
            d.ellipse([cx - size, cy - size, cx + size, cy + size], fill=255)
        elif t in _UNIT:
            d.polygon(_poly(cx, cy, size, ang, _UNIT[t]), fill=255)
        ...
        while not _cmd_queue.empty():
            try:
                ...
                if action == "set_material" and value in fs.fluid_properties:
                    ...
                    set_obstacle_mask(rasterize_shapes(cmd.get("shapes", [])))
