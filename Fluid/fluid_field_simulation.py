...
LONGPRESS_DURATION = 55.0
LONGPRESS_PRESSURE_THRESHOLD = 0.35
STALE_INPUT_TIMEOUT = 0.2   # 秒：共享内存帧计数超过此时长没变 → 判定设备已停发(手离开)，当作无触摸

# ── UI 配色（方案 B：冷灰底 / 墨蓝烟 / 金黄手，冷暖互补）──
# 换配色只改这四行即可；背景/烟/手三处都从这里取值。
PALETTE_BG   = (0.906, 0.914, 0.925)   # #E7E9EC 冷灰
SMOKE_COOL   = (0.137, 0.157, 0.200)   # #232833 墨蓝
SMOKE_WARM   = (0.192, 0.169, 0.141)   # #312B24 墨褐
PALETTE_HAND = (0.949, 0.729, 0.298)   # #F2BA4C 表情手（金黄肤色）
# 浅底主题：关闭为“黑底发光烟”设计的 bloom 后处理，避免整片背景被当高光糊掉。
LIGHT_THEME_POST = True
# 手周围接触阴影（AO）：手挡风→边界滞止→贴着轮廓一圈暗晕，把手“焊”进流场。
HAND_AO          = True
HAND_AO_STRENGTH = 0.25    # 晕圈最深处压暗比例（0~1）
HAND_AO_SCALE    = 3.0    # 衰减尺度(像素)：越大晕圈越宽越柔
...
def read_shared_memory():
    ...
    try:
        ...
        while True:
            try:
                mmf.seek(0)
                header = mmf.read(HEADER_SIZE)          # 不再跳过 header
                raw    = mmf.read(INPUT_FRAME_SIZE)
                if len(raw) >= INPUT_FRAME_SIZE and len(header) >= HEADER_SIZE:
                    frame_id = int(np.frombuffer(header[8:12], dtype='<i4')[0])   # C# 端 cnt
                    now_t = time.time()
                    changed = (frame_id != last_frame_id)
                    if changed:
                        last_frame_id = frame_id
                        last_fresh_t  = now_t
                    stale = (now_t - last_fresh_t) > STALE_INPUT_TIMEOUT
                    ...
                    touching = (contact_area > 5) and not stale
                    if contact_area and not stale:
                    ...
                    with shared_mask_lock:
                        ...
                        input_stale = stale
                        frozen_count = int(frozen_mask_np.sum())
                        ...
                        if touching and pressure >= LONGPRESS_PRESSURE_THRESHOLD and not longpress_latched:
                            ...
                            if freeze_progress >= 1.0:
                                ...
                                longpress_start = None
                                freeze_progress = 0.0
                                print("[touch] ★ 长按冻结已锁定，将持续驻留直到释放")   # 关键提示
                    ...
                    # 每秒心跳，无论有没有接触都打印 —— 用来看清残留到底是哪种成因
                    if now_t - last_log_t >= 1.0:
                        print(f"[touch] fid={frame_id} changed={changed} stale={stale} "
                              f"contact={contact_area} frozen={frozen_count} press={pressure:.2f}")
                        last_log_t = now_t
                time.sleep(1 / 30)
            except Exception as ex:
                print(f"[SharedMem] loop err: {ex!r}")   # 不再静默，异常直接打出来
                time.sleep(0.1)
...
def fill_color_bg():
    for i, j in color_field:
        color_field[i, j] = ti.Vector([PALETTE_BG[0], PALETTE_BG[1], PALETTE_BG[2]])
...
def paint_color_field(smokef: ti.template(), dyef: ti.template(),
    ...
    for i, j in smokef:
        if solidf[i, j] == 1:
            # 手整体统一主色，不做深色勾边
            colorf[i, j] = ti.Vector([PALETTE_HAND[0], PALETTE_HAND[1], PALETTE_HAND[2]])
            continue
        ...
        if density < 8.0e-5:
            colorf[i, j] = ti.Vector([PALETTE_BG[0], PALETTE_BG[1], PALETTE_BG[2]])
            continue
        ...
        # 深烟基色：density 高→偏墨褐(暖)，低→偏墨蓝(冷)，保留原冷暖过渡逻辑
        smoke_cool = ti.Vector([SMOKE_COOL[0], SMOKE_COOL[1], SMOKE_COOL[2]])
        smoke_warm = ti.Vector([SMOKE_WARM[0], SMOKE_WARM[1], SMOKE_WARM[2]])
        tone_mix   = ti.max(0.0, ti.min(1.0, 0.18 + 0.34 * density + 0.06 * noise_centered))
        smoke_col  = smoke_cool * (1.0 - tone_mix) + smoke_warm * tone_mix

        # 最实处再压深一点 → 体积感（深色 ×<1 = 更深）
        core = ti.max(0.0, ti.min(1.0, density * 1.35))
        smoke_col = smoke_col * (1.0 - 0.22 * core * (1.0 - 0.45 * stripe))

        # 纹理改为调制“覆盖率/不透明度”：稀薄、颗粒处更透→露出浅底，密实处更实
        grain = 0.90 + (0.18 * fine_weight) * noise_struct + (0.08 * fine_weight) * stripe
        alpha = ti.max(0.0, ti.min(1.0, val * grain))

        # 浅底 alpha 合成（吸收模型）：背景 * (1-α) + 烟 * α
        bg  = ti.Vector([PALETTE_BG[0], PALETTE_BG[1], PALETTE_BG[2]])
        col = bg * (1.0 - alpha) + smoke_col * alpha
...
def main(web_ui_only=False, max_frames=None, stats_json=None, initial_streamline=False):
    ...
    # 手部接触阴影缓存：AO 只依赖 _solid 掩膜，手不动就复用上一张。
    # 掩膜变化（process_mask_update 返回 True / 障碍物初始化）时置脏、重算。
    ao_factor_cache = None
    ao_dirty = True
    ...
    while web_ui_only or gui.running:
        ...
        if not paused:
            ...
            if visualize_karman:
                ...
                # 读取"是否有真实触摸"与"是否有长按冻结手印"（均在 reader 线程维护）
                with shared_mask_lock:
                    live_touch = has_touch_current
                    any_frozen = bool(frozen_mask_np.any())
                if not live_touch and not any_frozen:
                    # 手已离开、且没有冻结拓印 → 主动清空障碍，杜绝残留，
                    # 不再依赖上游是否推送"空帧"。用 solid_cleared 保证只清一次。
                    if not solid_cleared:
                        _solid.fill(0)
                        reset_mask_change_cache()   # 让去重缓存与 _solid 同步
                        ao_dirty = True
                        solid_cleared = True
                else:
            ...
            if should_render_frame:
                if visualize_c:
                    ...
                    if LIGHT_THEME_POST:
                        # 浅底：不能用“亮=烟”的 bloom（会把浅背景当高光糊掉）。
                        # 改成对深烟做轻微“暗部加深”提层次：暗处压得更暗，浅底基本不动。
                        lum = (0.2126 * frame_rgb[..., 0] + 0.7152 * frame_rgb[..., 1] + 0.0722 * frame_rgb[..., 2])[..., None]
                        dark_mask = np.clip(1.0 - lum / 0.55, 0.0, 1.0)  # 仅在较暗(烟)区域生效
                        frame_rgb = np.clip(frame_rgb * (1.0 - 0.10 * dark_mask), 0.0, 1.0)

                        # 手周围接触阴影：只在掩膜变化时重算，其余帧复用缓存（热路径仅一次乘法）
                        if HAND_AO:
                            if ao_dirty or ao_factor_cache is None:
                                solid_np = _solid.to_numpy()  # (res_x, res_y)，1=solid
                                if solid_np.max() > 0.5:
                                    fluid_u8 = (solid_np < 0.5).astype(np.uint8)          # 流体=1
                                    dist = cv2.distanceTransform(fluid_u8, cv2.DIST_L2, 3) # 每个流体格到最近手边的距离
                                    ao = np.exp(-dist / HAND_AO_SCALE).astype(np.float32)  # 贴边≈1，远处→0
                                    ao *= (solid_np < 0.5)                                 # 手内清零，避免压暗手
                                    ao_factor_cache = (1.0 - HAND_AO_STRENGTH * ao)[..., None]
                                else:
                                    ao_factor_cache = None                                 # 无障碍物→无需压暗
                                ao_dirty = False
                            if ao_factor_cache is not None:
                                frame_rgb = np.clip(frame_rgb * ao_factor_cache, 0.0, 1.0)

                        frame_rgb = np.power(np.clip(frame_rgb, 0, 1), 1.02)  # 极轻微加对比，避免发灰
                    else:
                        # 原黑底发光主题的 bloom + 提亮
                        lum = (0.2126 * frame_rgb[..., 0] + 0.7152 * frame_rgb[..., 1] + 0.0722 * frame_rgb[..., 2])[..., None]
            ...
            if gui is not None and visualize_c:
                ...
                gui.set_image(frame_rgb if frame_rgb is not None else color_field.to_numpy())
            ...
            if should_render_frame and vis_start_stream_line:
                ...
                # 箭头广播节流到 30fps（跟随帧编码节奏），不再随本地 GUI 每帧(60fps)刷爆网页 overlay
